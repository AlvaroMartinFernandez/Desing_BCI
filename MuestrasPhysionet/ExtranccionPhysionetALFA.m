...
c3delta= filter(Alfa,c3aux);
c4delta= filter(Alfa,c4aux);
czdelta= filter(Alfa,czaux);
%realizamos wavalet para bajar el numero de muestras para la extracion de
%caracteristicas
...
f=(f/(2*pi))*80;
figure(x+60);
plot(f,C3welchdelta,f,C4welchdelta,f,Czwelchdelta);
title('Evento3',tipoevento);
%Transformada de Fourier
...
c3beta= filter(Alfa,c3aux);
c4beta= filter(Alfa,c4aux);
czbeta= filter(Alfa,czaux);
%realizamos wavalet para bajar el numero de muestras para la extracion de
%caracteristicas
...
c3auxa= CanalC3(1,160*event+1:160*(event+4));
c4auxa= CanalC4(1,160*event+1:160*(event+1));
czauxa= CanalCz(1,160*event+1:160*(event+1));
c3a= filter(Alfa,c3auxa);
c4a= filter(Alfa,c4auxa);
cza= filter(Alfa,czauxa);
